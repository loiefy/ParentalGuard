...
### 4.4 Nội dung thông điệp overlay tuỳ biến (`FE-012`, bổ sung Đợt 6, ADR-110)

Phát hiện khi viết `10-ui-architecture.md` (Đợt 6, `S4` Cài đặt nâng cao): `FE-012` yêu cầu phụ huynh tự cấu hình được dòng chữ thông điệp hiển thị trên `ContentBlurOverlayForm` (`S7`), nhưng **chưa có kênh IPC/schema nào mang nội dung này từ `Service` xuống `Overlay`** — code Đợt 1 (`src/ParentalGuard.Overlay/Rendering/ContentBlurOverlayForm.cs`) hiện **hardcode** chuỗi `"Nội dung không phù hợp đã được che"` trực tiếp trong code (không qua `OverlayStrings.resx`, không khớp câu mặc định đã chốt ở `FE-012`: *"Nội dung nhạy cảm được phát hiện, hãy thoát nội dung để bảo vệ chính bạn."*) — vi phạm kép: (a) `FE-060` (cấm hardcode text), (b) `FE-012` (nội dung sai câu mặc định đã chốt). Nút "Tắt nội dung" (`closeButton.Text`) cũng hardcode tương tự, cùng lỗi (a) — `FE-012` yêu cầu nhãn nút **không** cho phụ huynh tự đổi nhưng **vẫn phải** qua hệ thống đa ngôn ngữ (`FE-060`), không phải ngoại lệ khỏi resource.

**Thiết kế bổ sung**:

1. **Message mới `OverlayMessageUpdate`** (field **66**, kênh Overlay 60-79, additive — đúng ADR-16, amendment `03-ipc-communication.md` cùng lượt): `{ string text }`. `Service → Overlay`, push ngay sau handshake (cùng thời điểm với `OverlayRectListCommand`/`MonitoringStatusUpdate`/`IconLayoutSync`, mục 4.3 ở `03-ipc-communication.md`) và bất kỳ khi nào phụ huynh đổi cấu hình qua `S4` (`ConfigUpdateRequest`, `10-ui-architecture.md`).
2. **`text` rỗng nghĩa là "dùng mặc định cục bộ"** — ngoại lệ có chủ đích duy nhất với nguyên tắc "`Service` dựng sẵn nội dung đầy đủ, `Overlay` không tự suy luận" (`03-ipc-communication.md` mục 3.2, áp dụng cho `ShowToastCommand`): giá trị mặc định của `FE-012` về bản chất là 1 hằng số **thuộc lớp resource/ngôn ngữ** (`FE-062`: *"Giá trị mặc định ban đầu... vẫn được lấy từ file resource ngôn ngữ hiện hành"*), không phải dữ liệu nghiệp vụ `Service` sở hữu — để `Overlay` tự đọc hằng số này từ chính `OverlayStrings.resx` cục bộ (thêm key `DefaultBlockedMessage`, giá trị đúng câu đã chốt ở `FE-012`) là lựa chọn nhất quán hơn buộc `Service` phải biết nội dung resource của 1 tiến trình khác. `config.db.monitoring_state.overlay_message` (field mới, amendment `04-data-architecture.md` cùng lượt) lưu chuỗi rỗng khi phụ huynh chưa từng tuỳ chỉnh — `Service` relay nguyên trạng (rỗng hoặc chuỗi đã tuỳ chỉnh), không tự chèn default vào đó.
3. **Validate ký tự (`FE-012a`)** thực hiện **2 lớp**: chặn nhập/dán ở `UI` (`10-ui-architecture.md`) **và** validate lại ở `Service` khi nhận `ConfigUpdateRequest` (không tin input UI, đúng mẫu hình đã áp dụng cho mọi input khác trong dự án — ví dụ chuẩn hoá Recovery Key ở `08` mục 6.2) — `Service` từ chối (giữ nguyên giá trị cũ, trả lỗi) nếu chuỗi chứa ký tự bị cấm hoặc vượt 255 ký tự, **không** tự động cắt bớt/lọc bỏ ký tự để "cố cứu" input không hợp lệ.
4. **Sửa lỗi Đợt 1 (bug, không phải amendment thiết kế)**: `ContentBlurOverlayForm` phải đổi sang dùng `OverlayStrings.BlockedMessage(string? overrideText)` (trả `overrideText` nếu không rỗng, ngược lại trả `DefaultBlockedMessage`) cho label nội dung, và `OverlayStrings.CloseButtonLabel` cho nút — cả 2 qua `.resx`, đúng `FE-060`. `OverlayCoordinator` giữ giá trị `text` nhận được từ `OverlayMessageUpdate` gần nhất (RAM, giống cách đã giữ `MonitoringStatusUpdate`/`IconLayoutSync`), áp dụng cho mọi `ContentBlurOverlayForm` mới dựng sau đó — không cần dựng lại các overlay đang hiển thị khi có `OverlayMessageUpdate` mới tới giữa chừng (thay đổi cấu hình overlay message không phải tình huống cần cập nhật real-time cho overlay đang che, khác `OverlayRectListCommand`).
...
| ADR-110 (v0.2.3) | Thông điệp overlay tuỳ biến (`FE-012`) qua message mới `OverlayMessageUpdate` (field 66) — chuỗi rỗng nghĩa là "dùng default cục bộ của `Overlay`" (ngoại lệ có chủ đích với nguyên tắc "Service dựng sẵn nội dung"), không phải `Service` gửi sẵn default | Default là hằng số resource/ngôn ngữ (`FE-062`) thuộc về `Overlay`, không phải dữ liệu nghiệp vụ `Service` sở hữu; tránh `Service` phải biết nội dung resource của tiến trình khác |
