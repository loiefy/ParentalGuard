...
> Version: v0.2.2 | Trạng thái: Approved | Cập nhật: 2026-09-24
...
- **Đợt 6 (`PERF-050b`, v0.2.2)**: cùng đúng cơ chế trên là điểm `Service` cắm "Chế độ hiệu năng" (`S4`, `10-ui-architecture.md` mục 6.4) vào — khi phụ huynh đổi `performance_mode`, `Service` chỉ cần gửi 1 `ControlVisionCommand` mới với `capture_interval_ms` tương ứng mapping đã chốt (`"balanced"`=2000ms/`"maximum_protection"`=1000ms — số liệu cụ thể và lý do ở `04-data-architecture.md` mục 3.3/ADR-127, không lặp lại ở đây), có hiệu lực ngay qua `wakeEvent.Set()`. Không cần sửa `CaptureLoopWorker`/`VisionRuntimeConfig` — đúng như thiết kế đã lường trước ở v0.1.0.
