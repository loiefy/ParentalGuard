...
> Version: v0.1.2 | Trạng thái: Approved | Cập nhật: 2026-09-19
...
|---|---|---|---|---|
| `*.exe` (5 executable: `Service`/`Watchdog`/`Vision`/`Overlay`/`UI`) | Full Control | **Modify** (không Full Control — đủ để installer/uninstaller elevated ghi đè bản cập nhật, không cấp thêm `WriteDAC`/`WriteOwner` tuỳ tiện) | **Read + Execute** | ACL **protected** (không kế thừa từ `%ProgramFiles%` cha — `SetAccessRuleProtection(true, false)` hoặc tương đương `icacls /inheritance:r`) |
...
- **Không có ngoại lệ Read cho Administrators** (khác với mục 4.1, nơi Administrators vẫn cần Modify để phục vụ cài đặt/gỡ hợp lệ) — vì dữ liệu ở đây là **dữ liệu nhạy cảm** (cấu hình giám sát, hash mật khẩu, log), không phải file thực thi; cài đặt/gỡ hợp lệ không cần đọc trực tiếp các file này (uninstaller chỉ cần quyền **xoá**, không cần đọc nội dung — `Service`/uninstaller chạy dưới ngữ cảnh SYSTEM cho thao tác này, chi tiết ở `07`).
