...
### 3a.7 Cảnh báo tần suất pause bất thường (`PAUSE-021`, ADR-109, bổ sung Đợt 6)

`PAUSE-021` đã được chủ dự án xác nhận trực tiếp **APPROVED** (`Specification/07-pause-resume-spec.md` v0.2.2, 2026-09-20 — ngưỡng chính thức **> 5 lần/ngày** theo lịch UTC, không còn là ví dụ minh hoạ) **sau khi** file này chốt v0.2.0 với câu hỏi mở còn treo ở mục 8 — mục này đóng câu hỏi mở đó, cụ thể hoá cơ chế tính toán phía `Service`. Đường hiển thị Dashboard cụ thể (banner, message IPC `DashboardSummaryQuery`/`AcknowledgePauseAnomalyRequest`) thiết kế đầy đủ ở `10-ui-architecture.md` (Đợt 6) — mục này chỉ định nghĩa **HOW phía `Service`** (khi nào tính, khi nào ghi audit log, khi nào set/xoá cờ chờ xác nhận), đúng đúng phân công đã dự kiến sẵn từ `04-data-architecture.md` mục 3.4 ("điểm tính toán... Dashboard `10-ui-architecture.md`, Đợt 6, hoặc quét lúc Service khởi động — đã sẵn sàng, chỉ còn thiếu đúng 1 con số ngưỡng").

**Thời điểm tính** — ngay sau bước 10 ở mục 3a.1 (ghi `audit.log: PauseActivated`), **trước** bước 11 (khởi động `PauseMonitor` tick):

1. Quét `audit.log` tìm toàn bộ record `event_type="PauseActivated"` có `ts_unix_ms` rơi vào đúng **ngày UTC hiện tại** (theo `trusted_now`, `08-password-authentication-architecture.md` mục 7.7/ADR-77) — quét trực tiếp, **không cache riêng** (đúng đã dự kiến ở `04` mục 3.4). `PauseActivated` là sự kiện hiếm (không phải hot-path capture/inference), quét toàn bộ 1 ngày lịch chấp nhận được về chi phí — không cần thêm hỗ trợ gì từ `audit_meta` (mục 3.6 ở `04`).
2. Đếm số record tìm được ở bước 1 (đã bao gồm chính record vừa ghi).
3. Nếu đếm được `> 5` **và** chưa từng ghi `PauseFrequencyAnomalyDetected` cho đúng ngày UTC này (quét thêm `event_type="PauseFrequencyAnomalyDetected"` có `detail.date_utc` khớp ngày hiện tại — tránh ghi trùng nếu phụ huynh tiếp tục pause thêm nhiều lần nữa sau khi đã vượt ngưỡng lần đầu trong cùng ngày, giữ đúng lần đầu vượt ngưỡng làm mốc lịch sử duy nhất/ngày):
   - Ghi thêm 1 record `audit.log`: `PauseFrequencyAnomalyDetected{date_utc, activation_count}` (schema đã định nghĩa ở `04-data-architecture.md` mục 5.1 từ v0.2.4) — ngay sau record `PauseActivated` vừa ghi (2 record liên tiếp cùng 1 lần kích hoạt).
   - Ghi `pause_state.anomaly_pending_ack = true` (field mới, `04-data-architecture.md` mục 3.4 amendment cùng lượt) vào `config.db`, đồng bộ, cùng lần ghi với bước 4 mục 3a.1 (không round-trip ghi riêng).
4. Nếu đã từng ghi `PauseFrequencyAnomalyDetected` cho đúng ngày hôm nay rồi (đợt bất thường đã ghi nhận) nhưng `anomaly_pending_ack` hiện đang `false` (phụ huynh đã bấm "Đã xem" từ trước đó trong ngày, rồi phụ huynh/ai đó lại tiếp tục pause thêm) — set lại `anomaly_pending_ack = true` (coi là nhắc lại, đáng được chú ý lần nữa) nhưng **không** ghi thêm record `audit.log` trùng lặp (giữ đúng nguyên tắc mục 3 — chỉ 1 record lịch sử/ngày).

**Xác nhận đã xem (`AcknowledgePauseAnomalyRequest`, Đợt 6)**: `Service` chỉ đặt `anomaly_pending_ack = false` khi nhận đúng message này từ `UI` (thiết kế đầy đủ ở `10-ui-architecture.md`) — **không** tự động set `false` chỉ vì có 1 phiên `UI` vừa kết nối (Dashboard có thể mở thoáng qua, chưa chắc phụ huynh đã kịp đọc banner). Đây thuần là cờ thông tin, không phải cơ chế bảo mật — không cần `action_token`/xác thực mật khẩu để xem hay xác nhận đã xem (đúng tinh thần `PAUSE-021`: "không phải để chặn hành vi... mà để tăng nhận thức").

**Khôi phục sau khi `Service` restart**: `anomaly_pending_ack` đọc lại nguyên trạng từ `config.db` ở bước `Starting` cùng lúc đọc `pause_state` (mục 3a.3 phần khôi phục) — không có logic đặc biệt nào khác, cờ này độc lập với việc đang `Running·Paused` hay không (một đợt bất thường có thể đã ghi nhận từ sáng, banner vẫn còn "chờ xem" dù hiện tại đã resume từ lâu).
...
| ADR-109 (v0.3.0) | Cảnh báo tần suất pause bất thường (`PAUSE-021`) tính bằng quét trực tiếp `audit.log` theo ngày UTC ngay tại thời điểm mỗi lần `PauseActivated`, không cache counter riêng; cờ `pause_state.anomaly_pending_ack` chỉ tắt khi `UI` chủ động gửi `AcknowledgePauseAnomalyRequest`, không tự tắt khi có phiên `UI` kết nối | Đúng dự kiến sẵn ở `04` mục 3.4 (không cache riêng); sự kiện hiếm nên quét trực tiếp chấp nhận được; cờ ack là tín hiệu "đã đọc" nên cần hành động rõ ràng của phụ huynh, không suy luận từ việc kết nối |
...
- [x] ~~`PAUSE-021` (cảnh báo tần suất pause bất thường) — gap trạng thái spec, CẦN `spec-maintainer`/chủ dự án quyết định TRƯỚC khi `feature-dev` implement phần này.~~ — **Đã xong** (chủ dự án APPROVED trực tiếp `PAUSE-021` với ngưỡng chính thức **> 5 lần/ngày**, `Specification/07-pause-resume-spec.md` v0.2.2, 2026-09-20). Cơ chế tính toán/ghi audit log/cờ xác nhận cụ thể hoá ở mục 3a.7 (bổ sung Đợt 6, ADR-109); đường hiển thị Dashboard ở `10-ui-architecture.md`.

## 9. Changelog file này

| Version | Ngày | Thay đổi |
|---|---|---|
| v0.3.0 | 2026-09-20 | MINOR — Đợt 6 (`ROADMAP.md`, Dashboard UI), amendment cùng lượt viết `10-ui-architecture.md`. Thêm mục 3a.7: đóng câu hỏi mở `PAUSE-021` còn treo ở mục 8 (bản v0.2.0) — chủ dự án đã APPROVED ngưỡng chính thức **> 5 lần/ngày** ở `Specification/07-pause-resume-spec.md` v0.2.2 (2026-09-20, sau khi file này chốt v0.2.0). Cụ thể hoá cơ chế phía `Service`: quét `audit.log` theo ngày UTC ngay tại mỗi lần `PauseActivated` (không cache counter riêng, đúng dự kiến `04` mục 3.4), ghi `PauseFrequencyAnomalyDetected` đúng 1 lần/ngày, cờ `pause_state.anomaly_pending_ack` (field mới, amendment `04` cùng lượt) chỉ tắt qua hành động tường minh `AcknowledgePauseAnomalyRequest` từ `UI` (thiết kế đầy đủ ở `10-ui-architecture.md`). 1 ADR mới (109). Theo chỉ đạo — không dừng chờ review |
