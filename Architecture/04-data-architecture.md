...
> Version: v0.4.0 | Trạng thái: Approved | Cập nhật: 2026-09-24
...
```json
{
  ...
  "risk_threshold": 0.0,
  "capture_interval_baseline_ms": 2000,
...
- `capture_interval_baseline_ms`: tần suất capture nền (`PERF-010`/`011`). **Từ Đợt 6 (v0.4.0), field này là GIÁ TRỊ DẪN XUẤT từ `performance_mode`** (xem bullet dưới) theo mapping cố định — `Service` luôn tự tính lại field này mỗi khi `performance_mode` được set (lúc khởi tạo lần đầu, lúc migrate 1 `config.db` cũ chưa có `performance_mode`, hoặc lúc nhận `ConfigUpdateRequest.performance_mode` mới) chứ **không còn** là 1 giá trị tự do UI/Service chỉnh độc lập như mô tả trước v0.4.0 — tránh 2 field lệch nhau (label mode nói 1 đằng, interval thực tế chạy 1 nẻo). Mapping (ADR-127, giá trị **đề xuất ban đầu, cần benchmark lại theo `PERF-061` như mọi ngưỡng hiệu năng khác trong dự án, không khoá cứng vào code**): `"balanced"` → **2000 ms** (giãn nhẹ so với hành vi mặc định hiện có, giảm tải CPU cho trường hợp phổ biến — đúng tinh thần "Cân bằng" là mức mặc định mới, ưu tiên trải nghiệm máy yếu hơn 1 chút so với trước); `"maximum_protection"` → **1000 ms** (giữ nguyên `MonitoringStateData.DefaultCaptureIntervalMs` hiện tại trong code — Đợt 1-5 luôn chạy ở mức này mặc định, nay trở thành lựa chọn rõ ràng thay vì hành vi ngầm định duy nhất). `feature-dev` cần cập nhật `MonitoringStateData` (giá trị khởi tạo dòng mới đổi từ 1000 sang 2000 do đổi mặc định `performance_mode="balanced"`) và thêm bước tính lại `capture_interval_baseline_ms` vào đúng 3 thời điểm nêu trên.
- `performance_mode` (mới v0.4.0, Đợt 6, `PERF-050b` — supersedes `PERF-050`/`PERF-050a` nay `DEPRECATED`, `Specification/08-performance-cpu-spec.md` v0.7.0): chuỗi literal `"balanced"` (mặc định) hoặc `"maximum_protection"` — **KHÔNG dùng enum số** (khác nhiều field khác trong file này) vì đây là field JSON nội bộ lưu trữ, ánh xạ 1-1 sang `enum PerformanceMode` phía IPC (`03-ipc-communication.md` mục 3.7/ADR-126) chỉ tại lớp chuyển đổi của `Service`, giữ JSON dễ đọc/debug trực tiếp (cùng tinh thần các field literal khác như `event_type` ở mục 5.1). **Fail-secure/backward-compat**: field **vắng mặt** (đọc 1 `config.db` tạo trước Đợt 6, hoặc dòng JSON cũ chưa từng có field này) → `Service` coi như `"balanced"` (mặc định an toàn — không phải giá trị "bảo vệ tối đa" vì đây không phải tình huống mất dữ liệu cần fail-secure nghiêm ngặt như `monitoring_enabled`/`exclude_process_names`, chỉ là 1 tham số tuning UX chưa từng tồn tại trước đó) **và ghi đè `capture_interval_baseline_ms` về 2000 ms ngay tại thời điểm đọc** (đồng bộ 2 field theo đúng mapping ở bullet trên, tránh để lại giá trị cũ 1000ms không khớp label "balanced"). Reset về `"balanced"` (+ `capture_interval_baseline_ms=2000`) khi rơi vào nhánh fail-secure `BE-061`/`ANTI-070` (mục 6.2) — nhất quán với các field cấu hình khác reset về giá trị an toàn/mặc định, **không** reset về `"maximum_protection"` dù nghe có vẻ "an toàn hơn": nhánh fail-secure đã tự đặt `risk_threshold`/`exclude_process_names`/whitelist về giá trị bảo vệ nghiêm ngặt nhất theo đúng nghĩa đen `ANTI-070` ("không có ngoại lệ nào được áp dụng") — `performance_mode` không nằm trong nhóm đó (không phải ngoại lệ giám sát, chỉ là tần suất), giữ nhất quán `"balanced"` = mặc định chuẩn cho mọi lần khởi tạo mới, tránh mở rộng phạm vi "an toàn nghiêm ngặt" ra ngoài đúng những gì `ANTI-070` liệt kê.
...
2. Nạp MonitoringState vào RAM bằng hard-code default:
   ...
   - performance_mode = "balanced"   (bổ sung v0.4.0, PERF-050b — mặc định chuẩn, không phải nhánh
     "an toàn nghiêm ngặt" riêng cho field này, xem giải thích ở mục 3.3)
   - capture_interval_baseline_ms = DEFAULT_CAPTURE_INTERVAL_MS (bổ sung v0.4.0: hằng số code nay =
     giá trị mapping của "balanced" = 2000ms, ADR-127, mục 3.3 — không còn con số Đợt 7 độc lập)
...
| ADR-127 (v0.4.0) | `capture_interval_baseline_ms` trở thành giá trị **dẫn xuất** từ `performance_mode` (mapping cố định: `"balanced"`→2000ms, `"maximum_protection"`→1000ms), `Service` tự tính lại chứ không nhận trực tiếp từ UI; field vắng mặt (`config.db` pre-Đợt 6) → suy ra `"balanced"` VÀ ghi đè interval khớp mapping ngay lúc đọc | Tránh trạng thái label mode và giá trị interval thực tế lệch nhau (ví dụ đọc field cũ giữ nguyên 1000ms nhưng label lại hiện "Cân bằng"); 2 con số **đề xuất ban đầu**, cần benchmark lại theo `PERF-061` như mọi ngưỡng hiệu năng khác trong dự án, không khoá cứng |
...
|---|---|---|
| v0.4.0 | 2026-09-24 | MINOR — amendment cùng lượt cập nhật `10-ui-architecture.md` v0.2.0 sau khi `spec-maintainer` chốt `PERF-050b` (`Specification/08-performance-cpu-spec.md` v0.7.0, supersedes `PERF-050`/`PERF-050a`, 2 mức "Cân bằng"/"Bảo vệ tối đa"). Thêm field `performance_mode` vào `monitoring_state` JSON (mục 3.3) — literal `"balanced"`/`"maximum_protection"`, mặc định `"balanced"` khi vắng mặt (backward-compat, không phải nhánh fail-secure nghiêm ngặt). Viết lại ngữ nghĩa `capture_interval_baseline_ms`: từ "field tự do, tuning ở Đợt 7" thành **giá trị dẫn xuất cố định từ `performance_mode`** (`"balanced"`=2000ms/`"maximum_protection"`=1000ms, giữ đúng hành vi mặc định hiện có của code Đợt 1-5) — `Service` tự đồng bộ lại cả 2 field mỗi khi đọc/migrate/nhận `ConfigUpdateRequest` mới, tránh lệch label↔giá trị thực. Cập nhật luồng fail-secure mục 6.2 bước 2 (reset `performance_mode="balanced"` + interval khớp mapping). 1 ADR mới (127). Amendment liên quan cùng lượt: `03-ipc-communication.md` v0.8.1 (`enum PerformanceMode`, field `performance_mode` trong `ConfigResponse`/`ConfigUpdateRequest`), `05-image-pipeline-architecture.md` v0.2.2 (1 dòng liên kết mapping), `10-ui-architecture.md` v0.2.0 (thiết kế UI `RadioButtons` ở `S4`, đóng câu hỏi mở) |
