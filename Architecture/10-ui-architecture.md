...
> Version: v0.2.0 | Trạng thái: Approved | Cập nhật: 2026-09-24
...
| S4 Đổi chế độ hiệu năng | `ConfigUpdateRequest` | Không (cosmetic/operational, `PERF-050b`, ADR-125, mục 6.4) | mục 6.4 file này |
...
- **Chế độ hiệu năng** (`PERF-050b`, `Specification/08-performance-cpu-spec.md` v0.7.0 — supersedes `PERF-050`/`PERF-050a`, nay `DEPRECATED`; câu hỏi mở tương ứng ở mục 11 đã đóng): `RadioButtons` (WinUI 3) 2 lựa chọn loại trừ lẫn nhau — "Cân bằng" (mặc định, chọn sẵn khi `ConfigResponse.performance_mode = BALANCED`) / "Bảo vệ tối đa" (`MAXIMUM_PROTECTION`) — ADR-125 (vì sao `RadioButtons` chứ không phải `ComboBox`). Đọc cùng lúc với `overlay_message`/`user_whitelisted_process_names` qua `ConfigQuery` (không round-trip riêng), ghi qua `ConfigUpdateRequest{overlay_message, performance_mode}` (UI luôn gửi đầy đủ giá trị hiện hành của cả 2 field, đúng nguyên tắc "full update" đã áp dụng cho `overlay_message`) — **không gate `S5`**, cosmetic/operational cùng nhóm với đổi thông điệp overlay ở trên (ADR-125 giải thích rõ vì sao khác `manage_whitelist` có gate). Đổi lựa chọn có hiệu lực ngay: `Service` ghi `monitoring_state.performance_mode` + `capture_interval_baseline_ms` tương ứng (mapping cụ thể ở `04-data-architecture.md` mục 3.3/ADR-127) rồi đẩy ngay `ControlVisionCommand` mới xuống `Vision` qua cơ chế `wakeEvent` đã có sẵn từ Đợt 1 (`05-image-pipeline-architecture.md` mục 3.3) — không cần restart `Service`/`Vision`, không cần thêm message IPC mới ngoài `performance_mode`. `Service` từ chối áp dụng nếu `ConfigUpdateRequest.performance_mode = PERFORMANCE_MODE_UNSPECIFIED` (giữ nguyên giá trị hiện có, an toàn — chi tiết `03-ipc-communication.md` mục 3.7); trường hợp này không thực tế xảy ra từ `RadioButtons` (luôn có 1 lựa chọn được chọn sẵn), chỉ là safety net phía `Service`.
...
| ADR-125 | `S4` "Chế độ hiệu năng" (`PERF-050b`) dùng `RadioButtons` (WinUI 3) 2 lựa chọn, đọc/ghi qua `ConfigQuery`/`ConfigUpdateRequest`, **không** gate `S5` | `RadioButtons` hiển thị đồng thời cả 2 lựa chọn rõ ràng hơn `ComboBox` ẩn trong dropdown (khuyến nghị Fluent Design cho tập ≤ 4-5 mục); không gate vì `PERF-050b` chỉ còn 2 mức đều nằm trong khung bảo vệ chấp nhận được — mức duy nhất có rủi ro giảm hiệu quả bảo vệ thực sự ("Tiết kiệm pin") đã bị loại bỏ hẳn ở cấp spec, không còn tồn tại để cần rào chắn xác thực; khác `MISC-030` (ADR-122, có gate) vì whitelist loại trừ HẲN 1 app khỏi giám sát (all-or-nothing), còn đổi mode chỉ điều chỉnh đồng đều tần suất adaptive áp dụng cho mọi app, không miễn trừ app nào khỏi giám sát |
...
|---|---|---|
| v0.2.0 | 2026-09-24 | **MINOR — đóng 2 câu hỏi mở còn treo ở mục 11 (v0.1.0), sau khi `spec-maintainer` xử lý xong ở `Specification/`.** (1) `PERF-050` → `PERF-050b` (`Specification/08-performance-cpu-spec.md` v0.7.0, ĐÃ CHỐT bởi chủ dự án — chỉ còn **2 mức** "Cân bằng"/"Bảo vệ tối đa", bỏ hẳn "Tiết kiệm pin" vì rủi ro giảm hiệu quả bảo vệ không được chấp nhận): viết thiết kế thật cho mục "Chế độ hiệu năng" ở `S4` (mục 6.4) — `RadioButtons` 2 lựa chọn, đọc/ghi qua `ConfigQuery`/`ConfigUpdateRequest.performance_mode`, **không gate `S5`** (cosmetic/operational, ADR-125), áp dụng ngay qua `ControlVisionCommand`/`wakeEvent` có sẵn từ Đợt 1. Thêm dòng bảng ánh xạ màn hình (mục 5) + ADR-125. (2) `MISC-030` (câu chữ "domain/app") — xác nhận `spec-maintainer` đã xử lý ở lượt trước, file này vốn đã chỉ thiết kế theo "app"/process name ngay từ đầu (mục 6.3/6.4 không cần sửa) — chỉ xoá bullet khỏi mục 11. Amendment cùng lượt: `03-ipc-communication.md` v0.8.0→**v0.8.1** (PATCH, enum `PerformanceMode` + field `performance_mode` vào `ConfigResponse`/`ConfigUpdateRequest` trong khối field 140-159 đã có sẵn, ADR-126), `04-data-architecture.md` v0.3.0→**v0.4.0** (MINOR, field `performance_mode` mới trong `monitoring_state` JSON — mặc định `"balanced"` khi vắng mặt/backward-compat; viết lại ngữ nghĩa `capture_interval_baseline_ms` thành giá trị **dẫn xuất** từ `performance_mode` theo mapping cố định — BALANCED=2000ms/MAXIMUM_PROTECTION=1000ms — thay vì field tự do như mô tả cũ, ADR-127), `05-image-pipeline-architecture.md` v0.2.1→**v0.2.2** (PATCH, 1 dòng liên kết mapping mode↔interval ở mục 3.3, trỏ về `04` là nguồn số liệu). Trạng thái file này: `Draft`→**`Approved`** (2 câu hỏi mở CHÍNH đã đóng, phần còn lại chỉ là 3 chi tiết implement không chặn thiết kế — cùng mức độ như các file `Approved` khác trong bộ tài liệu này). Không có gap mới nào phát sinh — không cần quay lại `spec-maintainer` thêm lần nữa |
